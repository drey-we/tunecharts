...
const WORKER_URL = "https://tunecharts-api.andry-we.workers.dev"; 
...
async function buscarPerfil() {
    try {
        const url = `${API_BASE}/api/?method=user.getinfo&user=${username}`;
...
async function buscarScrobblesDoPeriodo() {
    try {
        ...
        const url = `${API_BASE}/api/?method=user.getrecenttracks&user=${username}&limit=1&from=${fromDate}`;
        const res = await fetch(url);
        const data = await res.json();
        let total = "0";
...
async function buscarCharts(method, type, mainId) {
    try {
        const url = `${API_BASE}/api/?method=${method}&user=${username}&limit=10&period=${currentPeriod}`;
        const res = await fetch(url);
        const data = await res.json();
