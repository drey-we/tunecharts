...
let elements = {};
let globalTopArtistImage = ""; // Para guardar a imagem do Spotify
let spotifyTokenCache = null;  // Cache do token

// Dados Cacheados (Calculados)
let cachedData = {
    artists: [],
    tracks: []
};
...
async function carregarTudo() {
    elements = {
        ...
        scrobblesPerDay: document.getElementById("scrobblesPerDay"), // ID atualizado conforme novo HTML
        ...
        // Story Card Elements (Novo Design)
        ...
        // Square Card Elements (Novo Design V2)
...
async function atualizarDadosDoPeriodo(isInitialLoad = false) {
    ...
    // Reseta visual para Skeleton
    resetarChartsParaSkeleton();
    globalTopArtistImage = ""; // Limpa imagem antiga
    atualizarBanner(""); // Reseta banner

    // Inicia cálculo pesado
...
async function buscarImagemSpotify(artist, trackName, type) {
    ...
    // Remove (feat. X) ou - Remastered para melhorar busca
...
// --- CÁLCULO DE TEMPO (CORE LOGIC DO COUNTER) ---
async function calcularTempoOuvido() {
    try {
        ...
        // Counter precisa de mais dados para precisão, mantendo limit=1000
        const limit = 1000;
        ...
        tracks.forEach((track) => {
            let duration = parseInt(track.duration); // Duração em segundos fornecida pelo Last.fm
        ...
        // --- NOVIDADE: Buscar Capa do Top 1 Artista no Spotify ---
        if (sortedArtists.length > 0) {
            const topArtistName = sortedArtists[0].name;
            // Busca a imagem do artista Top 1
            buscarImagemSpotify(topArtistName, '', 'artist').then(url => {
                if (url) {
                    globalTopArtistImage = url;
                    atualizarBanner(url);
                    // Tenta atualizar a miniatura do item #1 na lista visual
        ...
        // Tenta buscar capa da Top 1 Track também
...
// Renderiza a lista HTML na página principal (NÃO é a imagem gerada)
function renderizarPreviewLista(elementId, dataList, type) {
    ...
    mainItems.forEach((item, i) => {
        ...
        const imgId = `img-${type}-${i}`; // ID único para inserção de imagem depois se precisar

        if (isTop1) {
            // Layout Especial Top 1 (Com Placeholder de Imagem)
            htmlMain += `
        ...
        elements.chartColOptions.forEach((checkbox) => {
            checkbox.onchange = () => {
                ...
                if (chartsToInclude.length > 2) {
                    checkbox.checked = false; // Impede marcar o terceiro
                    updateSelection();
                }
            };
        });
...
async function gerarImagemFinal(format, accentColor, selectedCharts) {
    ...
    // Configura container baseado no formato
    if (format === "story") {
    ...
    try {
        // Aplica cores e imagem de fundo no Story
        aplicarCoresDinamicas(cardElement, accentColor, format);

        // Espera renderizar styles e imagens
...
// Formata a lista PARA A IMAGEM (incluindo o tempo)
function formatarListaHTML(items, limit, type, format) {
    ...
    itemsToShow.forEach((item, i) => {
        ...
        if (format === "story") {
            // Layout Story (com tempo na direita)
            html += `
...
// Aplica cores e BACKGROUND IMAGE no card
function aplicarCoresDinamicas(card, accentColor, format) {
    if (format === "story") {
        ...
        // Cores de Fundo/Borda
        card.querySelectorAll(".story-stat, .story-item.top-1").forEach(el => {
            el.style.borderColor = accentColor;
            el.style.backgroundColor = accentColor + "33"; // 20% opacity
        });

        // Lógica de Background (Igual ao Charts)
        ...
        // Se tiver imagem do artista Top 1, aplica no Header com degradê
        if (headerElement) {
