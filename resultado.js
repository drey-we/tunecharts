...
function formatarListaHTML(items, limit, type, format) {
    ...
    itemsToShow.forEach((item, i) => {
        let text = item.name;
        if (format === "story") {
            html += `<div class="story-item ${i === 0 ? "top-1" : ""}"><span class="story-rank">#${i + 1}</span><span class="story-text">${text}</span></div>`;
        } else {
            let artistName = item.artist ? item.artist.name : "";
            if (type !== "artists" && artistName) text = `${item.name} - ${artistName}`;
...
function aplicarCoresDinamicas(card, accentColor, format) {
    if (format === "story") {
        ...
        card.style.background = `radial-gradient(circle 700px at top right, ${accentColor}66, transparent), #0f0f0f`;
